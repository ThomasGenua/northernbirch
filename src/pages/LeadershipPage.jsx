      ...
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        {tab==="overview"&&<div>
          <div style={{background:"#fff",borderRadius:24,padding:isMob?24:40,border:"1px solid #eee",marginBottom:24}}>
            ...
            <p style={{fontFamily:fs,fontSize:15,color:"#555",lineHeight:1.8,marginBottom:16}}>Today Northern Birch's insurance is travel cover by referral to Allianz Global Assistance and the insurance built into Collabria cards. Members who need life, home or auto insurance get it elsewhere, and whoever insures them has an opening to cross-sell banking.</p>
            <p style={{fontFamily:fs,fontSize:15,color:"#555",lineHeight:1.8,marginBottom:24}}>This proposal adds insurance referrals, AI-powered financial tools and business services to what Northern Birch already offers, so a small credit union can cover more of a member's financial life.</p>
            ...
            <p style={{fontFamily:fs,fontSize:12,color:"#6b5a2a",margin:"12px 0 0",lineHeight:1.6}}>Oodler's projections, not Northern Birch figures. The revenue depends on referral commissions that are still TO CONFIRM with each insurer; see the underwriter matrix.</p>
          </div>
          <div style={{background:`${C.navy}06`,borderRadius:20,padding:isMob?24:32,border:`1px solid ${C.navy}12`}}>
            ...
            <p style={{fontFamily:fs,fontSize:14,color:"#666",lineHeight:1.7,marginBottom:16}}>A working demonstration, not a mockup. What it shows:</p>
            <div className="grid-2-1" style={{gap:10}}>
              {["AI tools on Claude through a server-side proxy with referral-only rules, live once the API key is set","A Coverage Explorer that helps a member set out what they need, without quoting","English, Estonian and Latvian switching (translations not yet reviewed by a native speaker)","A demo member dashboard: balances, spending, documents and messages for a made-up member","Wire requests to the Baltics that a branch confirms, instead of online transfers","Claims guidance that sends members to the right insurer","Booking, applications and referrals that collect nothing in the demo","Build checks that stop wrong rates, phone numbers and unconfirmed insurers from shipping","Estonian/Latvian cultural branding (birch trees, cornflowers, daisies, folk patterns)","Tax & savings optimizer with RRSP/TFSA calculators"].map((f,i)=>
        ...
        {tab==="legal"&&<div>
          {[
            // Only what is known is stated as fact here. Regulatory mechanics --
            // what filing a referral arrangement needs, how long it takes, whether
            // a licensed route is open later -- are questions for counsel, and the
            // page says so instead of answering them.
            {title:"Insurance Distribution Model",color:C.accentText,content:"Under a referral model Northern Birch is not an insurer. It does not underwrite risk, handle claims or hold insurance capital; the insurer does. Northern Birch connects members with the insurer. Whether and how it is paid for referrals is TO CONFIRM with each insurer -- see the underwriter matrix.",items:["No underwriting risk to NBCU","No claims handling","No insurance capital","Revenue mechanic: TO CONFIRM"]},
            {title:"FSRA and Ontario's insurance rules",color:C.greenText,content:"Northern Birch is an Ontario credit union regulated by FSRA. Ontario's rules on the sale of insurance by credit unions limit what a credit union may sell itself, so the third-party lines in this proposal are referral-only. What a referral arrangement needs from FSRA, if anything, and how long it takes, is for counsel to confirm.",items:["Third-party lines: referral only","No licensing spend in phase one","Filing or approval: TO CONFIRM with counsel","Timeline: TO CONFIRM"]},
            {title:"RIBO (Registered Insurance Brokers of Ontario)",color:C.amberText,content:"RIBO licenses insurance brokers in Ontario. In a referral model the insurer's own licensed people advise and sell, so Northern Birch staff need no broker licence. Whether a licensed route is open to Northern Birch later, and what it would cost, is phase two of the ask and a question for counsel.",items:["Phase 1: referral, no licence","Phase 2: only if volume justifies it","Licensed route: TO CONFIRM with counsel","Cost of licensing: TO CONFIRM"]},
            {title:"PIPEDA (Privacy)",color:C.purple,content:"A referral would need the member's consent before anything is shared with an insurer, and members can opt out of insurance marketing. This demo collects nothing: its forms discard what they receive. Messages to its AI assistant are processed by Anthropic in the United States; a live service would have to disclose that, or keep the data in Canada.",items:["Consent before any referral data is shared","Opt-out of insurance marketing","Demo forms store nothing","AI messages processed in the US"]},
            {title:"FINTRAC (Anti-Money Laundering)",color:C.navy,content:"Northern Birch already meets FINTRAC obligations for its banking. Insurers handle their own. International wires stay in branch, as they are today, so the proposal adds no new money-transfer partner.",items:["Existing FINTRAC compliance covers banking","Insurers handle their own AML","Wires stay in branch","No new transfer partner"]},
            {title:"AODA (Accessibility)",color:C.redText,content:"Ontario's accessibility rules set WCAG 2.0 Level AA as the legal floor for websites; this site targets WCAG 2.1 Level AA, as the content review asked. Keyboard navigation, screen reader support, colour contrast and resizable text are tested automatically on every page.",items:["WCAG 2.1 Level AA target","Keyboard and screen reader support","Accessible formats on request","Staff training on accessible service"]},
          ].map((section,i)=>(
          ...
          <div className="grid-2-1" style={{gap:16,marginBottom:24}}>
            <div style={{background:"#fff",borderRadius:20,padding:28,border:"1px solid #eee"}}>
              ...
              {[
                ...
                {l:"Home & Auto Insurance (proposed: The Personal)",v:"C$50K-$400K/yr",d:"Group rates, if negotiated, drive adoption"},
                {l:"Travel Insurance",v:"C$10K-$75K/yr",d:"Baltic community = high travel frequency"},
                {l:"Group Benefits (carrier to choose)",v:"C$15K-$150K/yr",d:"5-10 business accounts per year"},
                {l:"Additional Revenue Streams",v:"C$10K-$300K/yr",d:"International wires, foreign cash, referrals"},
              ].map((r,i)=>(
        ...
        {tab==="competitive"&&<div>
          <div style={{background:"#fff",borderRadius:24,padding:isMob?24:40,border:"1px solid #eee",marginBottom:24}}>
            ...
            {[
              ...
              {title:"Co-op & Co-ownership Mortgages",desc:"Northern Birch is one of the few Ontario institutions with both co-op mortgage expertise AND co-op mortgage lending products. This is a genuinely ownable niche.",color:C.accentText},
        ...
        {tab==="implementation"&&<div>
          <div style={{background:"#fff",borderRadius:24,padding:isMob?24:40,border:"1px solid #eee",marginBottom:24}}>
            ...
            {[
              {phase:"Month 1-2",title:"Discovery & Partner Selection",color:C.accentText,items:["Insurance partner selection (CUMIS, The Personal; group benefits carrier to choose)","Commission structure negotiation","Regulatory review with FSRA","Member needs survey","KESKUS timeline alignment"]},
              {phase:"Month 2-3",title:"Licensing & Compliance",color:C.greenText,items:["Regulatory filing, if counsel says one is needed","Referral agreement with insurers","Privacy assessment (PIPEDA)","Compliance framework established","Staff training program begins"]},
              {phase:"Month 3-6",title:"Phase 1 Launch: Creditor & Life",color:C.amberText,items:["Creditor insurance embedded in mortgage applications","Term life and critical illness referrals begin","Staff trained on making referrals","KESKUS pre-launch marketing","Digital platform soft launch"]},
              {phase:"Month 6-10",title:"Phase 2: Home, Auto & Travel",color:C.purple,items:["Home and auto referrals to the chosen insurer","Co-op and co-ownership lending featured online","Auto insurance referrals active","Allianz travel referrals featured online","Tenant insurance referrals"]},
              {phase:"Month 10-14",title:"Phase 3: Group Benefits & Digital",color:C.navy,items:["Group benefits referrals for business members","Commercial property & liability insurance","Wire requests from online banking, confirmed in branch","Member dashboard in online banking","AI advisor launched"]},
              {phase:"Month 14-18",title:"Optimization & Expansion",color:C.redText,items:["Phase two licensing decision (if volume justifies it)","Estates administration content","Payroll partnerships active","Cross-sell analytics and optimization","Board performance reporting"]},
            ].map((phase,i)=>(
